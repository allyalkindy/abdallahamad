...
function UpdateTreatment({ isOpen, onClose, onSuccess, treatment }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/treatment/${treatment._id}`, formData, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      onSuccess();
      onClose();
    } catch (err) {
      setError(err.response?.data?.message || 'Error updating treatment');
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-gray-800 rounded-lg p-6 w-full max-w-2xl my-8 relative max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-white">Update Treatment</h2>
        </div>
